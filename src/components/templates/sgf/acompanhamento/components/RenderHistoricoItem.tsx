...
export function RenderHistoricoItem({item, idx, tipo}: Props) {
    ...
    return (
       <>
           {item.titulo && (
               <div key={`${tipo}-${idx}`} className={`flex items-start gap-4 p-4 rounded-xl border dark:bg-[#0d0d0d] dark:border-white/15 ${anexoConfig.bgColor}`}>

                   <div className="flex-1">
                       {/* Header */}
                       <div className="flex items-center justify-between mb-1">
                           <div className='flex items-center gap-2'>
                               <div className={`w-10 h-10 rounded-full flex items-center justify-center border ${anexoConfig.bgColor}`}>
